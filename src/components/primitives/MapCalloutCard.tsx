...
export interface MapCalloutCardProps {
  ...
  onAddPost?: () => void;
  onClose?: () => void;
  /** When provided, the name/address area becomes tappable to open the place. */
  onOpen?: () => void;
  /** If true, positions absolutely within map container. Default: fixed to viewport */
  anchoredToMap?: boolean;
}

export function MapCalloutCard({ place, onSave, onAddPost, onClose, onOpen, anchoredToMap = false }: MapCalloutCardProps) {
    ...
    <div className={positionClass}>
      <CardShell variant="glass" className="overflow-hidden">
        <div className="flex gap-3">
          ...
          {/* Content — tappable to open when onOpen is provided */}
          {(() => {
            const inner = (
              <>
                <h3 className="font-semibold text-title text-sm mb-1 truncate">{place.name}</h3>
                <div className="flex items-center text-body text-xs mb-2">
                  <MapPinIcon className="w-3 h-3 mr-1 flex-shrink-0" />
                  <span className="truncate">{place.address}</span>
                </div>
                {place.distance && (
                  <div className="text-meta text-xs font-medium">{place.distance}</div>
                )}
                {place.tags && place.tags.length > 0 && (
                  <div className="flex gap-1 mt-2 flex-wrap">
                    {place.tags.slice(0, 2).map(tag => (
                      <span key={tag} className="px-2 py-0.5 bg-aurum-200/50 text-bark-900 text-xs rounded-full border border-aurum-200/70">
                        #{tag}
                      </span>
                    ))}
                  </div>
            ...
            return onOpen ? (
              <button type="button" onClick={onOpen} className="flex-1 min-w-0 text-left press" aria-label={`Open ${place.name}`}>
                {inner}
              </button>
            ) : (
